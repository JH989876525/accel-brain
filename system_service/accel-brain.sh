...
bring_up_docker0_bridge() {
    info "Bringing up docker0 bridge ..."

    ip link add ppes.veth0 type veth peer ppes.veth1 && sleep 1
    ip link set dev ppes.veth0 up
    ip link set dev ppes.veth1 up

    ip link set dev ppes.veth0 master docker0 || die "Failed to bring up docker0 bridge ..."
}
